...
for (p in 1:length(pep)) {
  ...
  elements <- work_fy %>%
    filter(staff == peep) %>%
    select(performance_plan_element) %>%
    unique() %>%
    arrange(performance_plan_element)
  
  for (e in 1:nrow(elements)) {

    element <- elements$performance_plan_element[e]
    cat("\n\n### ", if(element == 0) {"---Project Lead---"} else {paste0("---Element #", element, "---")}, "\n")
    
    project_parents <- work_fy %>%
      filter(staff == peep,
             performance_plan_element == element) %>%
