...
```{r overdue_work}
knitr::kable(overdue, format = "html")
```
...
```{r figures_byStaff, results = 'all', fig.width = 10}
# Figure 1
for (i in 1:length(staff)) {
  ...
  fig <- ggplot(figure_staff, aes(x = adjusted_start, xend = adjusted_end, 
    ...
    scale_color_manual(values = c("Milestone" = "cadetblue",
                                ...
                                "Other" = "darkslategray")) + 
    scale_fill_discrete(limits=c("Milestone",
                                "Prep/Planning",
                                "Development",
                                "Fieldwork/Travel",
                                "Dataset",
                                ...
                                "Report/Manuscript",
                                "Other")) +
    scale_pattern_manual(name = "Status",
                         values = c(
                           "In progress" = "none",
                           "Awaiting dependency" = "wave", 
                           "Not yet started" = "stripe")) +
  ...
  for (c in 1:nrow(component)) {
    ...
    for (p in 1:nrow(project)) {
      figure_project <- planning %>% 
        ...
        arrange(adjusted_start, adjusted_end, activity_name)
      ...
      fig <- ggplot(figure_project, aes(x = adjusted_start, xend = adjusted_end,
                                       ...
                                       label = activity_lead)) +
        geom_segment(size = 5) +
        geom_vline(aes(xintercept = Sys.Date()), col = "black", size = 1.5) +
        scale_color_manual(values = c("Not yet started" = "darkslategray",
                            "Awaiting dependency" = "darkgoldenrod2",
                            "In progress" = "chartreuse4")) +
