...
priorities <- RPostgreSQL::dbGetQuery(con, "SELECT * FROM administrative.qry_project_priorities") 

p_critical <- priorities %>%
  filter(project_priority == "Mission Critical") %>%
  select(-project_priority) %>%
  arrange(activity_plan_nickname, project_name) %>%
  rename("Activity Plan" = activity_plan_nickname,
         "Project" = project_name,
         "Status" = project_status)

p_high <- priorities %>%
  filter(project_priority == "High Priority") %>%
  select(-project_priority) %>%
  arrange(activity_plan_nickname, project_name) %>%
  rename("Activity Plan" = activity_plan_nickname,
         "Project" = project_name,
...
work_fy_priorities <- work_fy %>%
  filter(project_priority != 'Other')
...
planning <- work %>%
  ...
  mutate(work_intensity = factor(work_intensity, levels=c("High", "Moderate", "Low", "Unknown"), ordered = TRUE)) %>%
  mutate(project_priority = factor(project_priority, levels=c("Mission Critical", "High Priority", "Other", "Unknown"), ordered = TRUE)) %>%
...
The following tables list projects by their priority level.
...
```{r mission_critical, results='asis'}
knitr::kable(p_critical, format = "pipe", padding = 2)
```

## High Priority Projects
```{r high_priority, results='asis'}
knitr::kable(p_high, format = "pipe", padding = 2)
```
:::
...
for (i in 1:length(pep)) {
  ...
  fig <- ggplot(figure_staff, aes(x = adjusted_start, xend = adjusted_end, y = ylab, yend = ylab,
                                  color = performance_plan_element, alpha = project_priority)) +
    ...
    scale_color_manual(name = "Performance Plan",
                       values = c("Project lead" = "deeppink4",
                                  ...
                                  "Elem#4" = "cadetblue",
                                  "Elem#5" = "orchid4")) +
      scale_alpha_manual(name = "Project Priority",
                       values = c("Mission Critical" = 1,
                                  "High Priority" = 0.45,
                                  "X" = 0.1,
                                  "Unknown" = 0)) +
    # scale_alpha_manual(name = "Work Intensity", # Using work intensity field
    #                    values = c("High" = 1,
    #                               "Moderate" = 0.45,
    #                               "Low" = 0.1,
    #                               "Unknown" = 0)) +
...
The following tables list the actions associated with each planned manuscript. Asterisks at the end of each role indicate project priority (one asterisk = mission critical work; two asterisks = high priority work).
...
# Prioritized Performance Plan Elements (FY26)

The following tables list the roles associated with Mission Critical or Top Priority projects for all PEP staff by performance plan element.

<details>

<summary>Click to Expand</summary>

```{r element_priorities, results='asis'}
cat("::: {.panel-tabset}")
for (p in 1:length(pep)) {
  peep <- pep[p]
  cat("\n\n## ", peep, "\n")
  ...
  for (e in 1:nrow(elements)) {
    ...
    project_parents <- work_fy_priorities %>%
      filter(staff == peep,
             pp == element) %>%
      select(project_parents) %>%
      unique() %>%
